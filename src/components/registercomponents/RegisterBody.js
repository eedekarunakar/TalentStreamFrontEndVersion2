...
const isEmailValid = (email) => {
  ...
  const isCompanyValid = (companyname) => {
  // Company name must not be empty
  if (!companyname) {
    return false;
  }
 
  // Company name must be at least 6 characters long
  ...
  const isFormValid = () => {
    ...
    if (!isPasswordValid(password)) {
      setErrorMessage('Password must be at least 6 characters long and should have one capital letter and one small letter and no spaces are allowed.');
      return false;
    }
    if (!isCompanyValid(companyname)) {
    setErrorMessage('Please enter a valid company name (at least 6 characters).');
    return false;
  }
